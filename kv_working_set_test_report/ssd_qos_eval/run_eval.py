#!/usr/bin/env python3
"""Poisson QPS-knee eval: all-GPU vs hierarchical, plus SLC/MLC/N3 and qd_cap."""
...
def plot_official(all_gpu: dict, hier: dict) -> None:
    ...
    for ax, key, ylabel, scale in (
        ...
        ax.legend()
    fig.suptitle("Poisson QPS knee (128KiB DMA + layer prefetch)")
    ...
    for ax, label, g, h in zip(axes, labels, gpu_vals, hier_vals):
        ...
        ax.set_title(label)
    fig.suptitle("Official knee metrics")
...
def plot_qos(rows: list[dict]) -> None:
    ...
    ax.set_ylabel("λ* (r/s)")
    ax.legend()
    ax.set_title("qd_cap sweep knee QPS (SLC vs N3, 128KiB)")
...
def main() -> int:
    ...
    (REPORT / "all_gpu_poisson.json").write_text(
        json.dumps(all_gpu, indent=2) + "\n"
    )

    hier = find_config_knee(
        "hier_slc",
        REPORT / "hier_poisson",
