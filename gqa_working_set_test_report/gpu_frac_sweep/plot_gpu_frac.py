#!/usr/bin/env python3
"""Figures for gqa_gpu_frac_inference_speed.md."""
...
from bench_lib import cache_gpu_blocks, peak_b as lib_peak_b  # noqa: E402
...
def main() -> None:
    ...
    b_dec = [int(r["peak_b"]) for r in rows]
    b_pre = [lib_peak_b(1.0, context=512, gpu_blocks=GPU_BLOCKS) for _ in rows]
    ...
    avail = GPU_BLOCKS - int(0.01 * GPU_BLOCKS)
    ...
    ax2.set_ylim(0, max(b_dec) * 1.15 if b_dec else 1)
    ...
    ax.set_ylabel("System token/s at Poisson knee")
    ax.set_title("GQA-8 knee token/s vs peak B")
    fig.tight_layout()
    ...
    ax.set_title(f"GQA-8 prefill vs decode peak B — prefill full-context (B={b_pre[0]} at S=512)")
